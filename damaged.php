...
if(isset($_POST['update'])){
  extract($_POST);
  $sql = "UPDATE `damaged` SET `slno`='$slno', `pcode`='$pcode', `cname`='$cname', `pname`='$pname', `quantity`='$quantity', `reportdate`='$reportdate' WHERE `id`='$update'";
  if($mysqli->query($sql)){
...
if(isset($_POST['delete'])){
  extract($_POST);
  $sql = "DELETE FROM `damaged` WHERE `id`='$delete'";
  if($mysqli->query($sql)){
...
<head>
  <meta charset="utf-8">
  <title>DAMAGED-STOCK-ENTRY</title>
...
<body>
  ...
  <h3><b><u>DAMAGED STOCK RECORD&nbsp<input type="text" id="myInput" onkeyup="myFunction()" placeholder="🔍Search your data..." title="Type in a name"></u></b></h3>
...
<center><div style=" color: white;">
  <?php
include('damagedform.php');
?>
   <button class="button1" onclick="document.getElementById('id05').style.display='block'" style="width:auto;">Enter Record</button> &nbsp&nbsp&nbsp&nbsp&nbsp
<button class="button1" 
    onclick="window.location.href = 'damagedprint.php';" style="width:auto;">
        Print Preview
    </button>  
</div></center>
<table class="table" id="table">
  ...
  <thead>
    <tr>
      ...
      <th scope="col">Damage Report Date</th>
...
<div class="modal fade" id="exampleModal-<?php echo $row['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-<?php echo $row['id'] ?>" aria-hidden="true">
 ...
 <div class="form-group">
    <label>Damage Report Date</label>
    <input type="date" class="form-control" name="reportdate" id="reportdate-<?php echo $row['id'] ?>" value="<?php echo $row['reportdate']; ?>" >
