...
if(isset($_POST['update'])){
  extract($_POST);
  $sql = "UPDATE `reused` SET `slno`='$slno', `pcode`='$pcode', `cname`='$cname', `pname`='$pname', `quantity`='$quantity', `rate`='$rate' WHERE `id`='$update'";
  if($mysqli->query($sql)){
...
if(isset($_POST['delete'])){
  extract($_POST);
  $sql = "DELETE FROM `reused` WHERE `id`='$delete'";
  if($mysqli->query($sql)){
...
<head>
  <meta charset="utf-8">
  <title>REUSED-STOCK-ENTRY</title>
...
<body>
  ...
  <h3><b><u>REUSED STOCK RECORD&nbsp<input type="text" id="myInput" onkeyup="myFunction()" placeholder="🔍Search your data..." title="Type in a name"></u></b></h3>
...
<center><div style=" color: white;">
  <?php
include('reusedform.php');
?>
   <button class="button1" onclick="document.getElementById('id03').style.display='block'" style="width:auto;">Enter Record</button> &nbsp&nbsp&nbsp&nbsp&nbsp
<button class="button1" 
    onclick="window.location.href = 'reusedprint.php';" style="width:auto;">
        Print Preview
    </button>  
</div></center>
...
<div class="modal fade" id="exampleModal-<?php echo $row['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-<?php echo $row['id'] ?>" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="reused.php" method="POST">
